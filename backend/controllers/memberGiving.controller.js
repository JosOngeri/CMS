...
class MemberGivingController extends BaseController {
  ...
  async getMemberGivingHistory(req, res) {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to fetch member giving history' });
    }
  }
  ...
  async getMemberGivingSummary(req, res) {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to fetch member giving summary' });
    }
  }
  ...
  async getMemberGivingTrends(req, res) {
    try {
      ...
      res.json({ success: true, data, year: targetYear, period });
    } catch (error) {
      this.logger.error('getMemberGivingTrends', error);
      res.status(500).json({ success: false, error: 'Failed to fetch member giving trends' });
    }
  }
  ...
  async getTopGivers(req, res) {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to fetch top givers' });
    }
  }
  ...
  async getMemberGivingComparison(req, res) {
    try {
      ...
      res.json({ success: true, data: comparison, currentYear, previousYear });
    } catch (error) {
      this.logger.error('getMemberGivingComparison', error);
      res.status(500).json({ success: false, error: 'Failed to fetch member giving comparison' });
    }
  }
  ...
  async getGivingByDepartment(req, res) {
    try {
      ...
      res.status(500).json({ success: false, error: 'Failed to fetch giving by department' });
    }
  }
}
