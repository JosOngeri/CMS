...
class SettingsRepository extends BaseRepository {
  ...
  async getSettingByKeySimple(key) {
    const result = await this.pool.query(
  ...
  async updateSettingValue(key, value) {
    const result = await this.pool.query(
  ...
  async createSettingSimple(key, value, label) {
    const result = await this.pool.query(
      `INSERT INTO settings (key, value, value_type, category, label, is_public, is_editable)
       VALUES ($1, $2, 'string', 'appearance', $3, true, true)
       RETURNING *`,
      [key, value, label]
    );
    return result.rows[0];
