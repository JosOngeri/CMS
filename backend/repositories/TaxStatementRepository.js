...
class TaxStatementRepository extends BaseRepository {
  ...
  async getAllTaxStatements(filters = {}) {
  ...
  async checkExistingStatement(memberId, taxYear) {
    const result = await this.pool.query(
      'SELECT id FROM tax_statements WHERE member_id = $1 AND tax_year = $2',
      [memberId, taxYear]
    );
  ...
  async getTaxDeductiblePayments(memberId, taxYear) {
    const result = await this.pool.query(
  ...
  async createTaxStatement(client, data) {
    const { member_id, tax_year, total_amount, generated_by } = data;

    const result = await client.query(
      `INSERT INTO ${this.tableName} (member_id, tax_year, total_amount, status, generated_by)
       VALUES ($1, $2, $3, 'generated', $4)
       RETURNING *`,
      [member_id, tax_year, total_amount, generated_by]
    );
    return result.rows[0];
  }
  ...
  async generateTaxStatement(memberId, taxYear, generatedBy) {
    const client = await this.beginTransaction();
    try {
      const existing = await this.checkExistingStatement(memberId, taxYear);

      if (existing) {
        await this.rollbackTransaction(client);
        return { error: 'Tax statement already exists for this member and year' };
      }
      ...
      const payments = await this.getTaxDeductiblePayments(memberId, taxYear);
  ...
  async regenerateTaxStatement(id, regeneratedBy) {
    const client = await this.beginTransaction();
    try {
      ...
      if (!statement) {
        await this.rollbackTransaction(client);
        return { error: 'Tax statement not found' };
      }

      await this.deleteTaxStatementItems(client, id);

      const payments = await this.getTaxDeductiblePayments(statement.member_id, statement.tax_year);
      const totalAmount = payments.reduce((sum, p) => sum + parseFloat(p.amount), 0);
