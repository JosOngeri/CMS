...
class ReconciliationService {
   ...
   */
  async reconcilePayment(paymentId, transaction) {
    try {
      // Get payment details
      ...
      if (match.isMatch) {
        ...
        await this.markPaymentReconciled(paymentId, transaction.transaction_id, match.discrepancy);
        
        // Update transaction as matched
        await this.markTransactionMatched(transaction.transaction_id, paymentId);
   ...
   */
  async markPaymentReconciled(paymentId, transactionId, discrepancy) {
    const query = `
      UPDATE payments
      ...
      WHERE id = $1
    `;
    await pool.query(query, [paymentId, transactionId, discrepancy]);
  }
   ...
   * @param {string} paymentId - Payment ID
   */
  async markTransactionMatched(transactionId, paymentId) {
    const query = `
    ...
    `;
    await pool.query(query, [transactionId, paymentId]);
  }
  ...
  async getReconciliationStats(churchId, filters = {}) {
    ...
    const query = `
      SELECT
        ...
        SUM(amount) FILTER (WHERE reconciled IS NULL) as unreconciled_amount
      FROM payments
      WHERE ${conditions.join(' AND ')}
    `;
