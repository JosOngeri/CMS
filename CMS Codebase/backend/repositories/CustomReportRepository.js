...
class CustomReportRepository extends BaseRepository {
  ...
  async generateReport(reportId, parameters = {}) {
    ...
    const reportResult = await this.getReportDefinition(reportId);

    if (reportResult.length === 0) {
      throw new Error('Report not found');
    }

    // Extract report definition from first row
    const reportDefinition = {
      data_source: reportResult[0].data_source,
      group_by: reportResult[0].group_by,
      order_by: reportResult[0].order_by
    };

    // Extract columns
    const columns = reportResult
    ...
    // Build secure query using QueryBuilderService
    const { query, params } = this.queryBuilder.buildSelectQuery(
      {
        ...reportDefinition,
        columns,
        filters
    ...
    return {
      report_name: reportResult[0].report_name,
      report_type: reportResult[0].report_type,
