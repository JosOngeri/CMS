...
class AnnouncementsRepository extends BaseRepository {
  ...
  async checkAnnouncementAccess(userId, userRoles, announcement) {
    // If announcement is public, allow access
    ...
    if (announcement.department_id) {
      const deptMember = await this.checkDepartmentMember(userId, announcement.department_id);
      if (deptMember) {
        return true;
      }
    }
