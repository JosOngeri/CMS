...
class AnalyticsRepository extends BaseRepository {
  ...
  async getMemberDemographics(churchId = null) {
    let query = `
      SELECT
        ...
        COUNT(CASE WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, date_of_birth)) < 18 THEN 1 END) as age_0_17,
        COUNT(CASE WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, date_of_birth)) BETWEEN 18 AND 25 THEN 1 END) as age_18_25,
        COUNT(CASE WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, date_of_birth)) BETWEEN 26 AND 35 THEN 1 END) as age_26_35,
        COUNT(CASE WHEN EXTRACT(YEAR FROM AGE(CURRENT_DATE, date_of_birth)) BETWEEN 36 AND 50 THEN 1 END) as age_36_50,
  ...
  async getUserActivity(days = 30, churchId = null) {
    let query = `
      SELECT
        ...
        COUNT(CASE WHEN last_login >= CURRENT_DATE - INTERVAL '1 day' THEN 1 END) as daily_active,
  ...
  async getContentViews(churchId = null) {
    ...
    query += ` ORDER BY view_count DESC, c.created_at DESC`;
  ...
  async getHeatmapData(period = '7d', churchId = null) {
    ...
    query += ` GROUP BY EXTRACT(HOUR FROM created_at), EXTRACT(DOW FROM created_at) ORDER BY day_of_week, hour`;
