...
// In-memory rate limiting for MFA attempts
// In production, this should be replaced with Redis
const mfaFailedAttempts = new Map();
const MAX_MFA_ATTEMPTS = 5;
const MFA_LOCKOUT_DURATION = 15 * 60 * 1000; // 15 minutes

// In-process cache for getIdentity with 60-second TTL
const identityCache = new Map();
const IDENTITY_CACHE_TTL = 60 * 1000; // 60 seconds
...
class IdentityService {
  ...
  invalidateIdentityCache(userId) {
    ...
    logger.debug(`Identity cache invalidated for user ${userId}`);
  }
  ...
  async validateMFA(identity, token) {
    ...
    const attempts = mfaFailedAttempts.get(userId);
    if (attempts && attempts.count >= MAX_MFA_ATTEMPTS) {
      const timeSinceLastAttempt = Date.now() - attempts.lastAttempt;
      if (timeSinceLastAttempt < MFA_LOCKOUT_DURATION) {
        logger.warn(`MFA locked out for user ${userId}`);
    ...
    if (!isValid) {
      const currentAttempts = mfaFailedAttempts.get(userId) || { count: 0, lastAttempt: 0 };
      mfaFailedAttempts.set(userId, {
        count: currentAttempts.count + 1,
        lastAttempt: Date.now()
      });
      logger.warn(`MFA validation failed for user ${userId}, attempt ${currentAttempts.count + 1}/${MAX_MFA_ATTEMPTS}`);
    } else {
