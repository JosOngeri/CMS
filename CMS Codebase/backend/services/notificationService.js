...
class NotificationService {
  ...
  async createNotification(data, churchId) {
    ...
    if (data.title && data.title.length > 255) {
      throw new Error('Notification title exceeds maximum length of 255 characters');
    }
    if (data.message && data.message.length > 2000) {
      throw new Error('Notification message exceeds maximum length of 2000 characters');
    }
