...
function Sidebar({ isOpen, setIsOpen }) {
  ...
  const menuItems = [
    ...
    { path: '/dashboard/payments', icon: DollarSign, label: 'Treasury' },
  ...
  const finalMenuItems = menuItems.filter(item => {
    ...
    // Treasury for Treasurer role
    if (item.path === '/dashboard/payments' && user?.roles?.includes('Treasurer')) return true;

    // Events for roles that can see them (logic would be in canAccessModule)

    const requiredPermissions = MODULE_PERMISSIONS[item.path];
