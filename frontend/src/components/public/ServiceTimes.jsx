...
const ServiceTimes = () => {
  ...
  const addToCalendar = (serviceName, time) => {
    ...
    const details = `Join us for ${serviceName} at ${churchName}`;

    const today = new Date();
    const date = today.toISOString().split('T')[0];
    const googleCalendarUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(title)}&dates=${date}/${date}&details=${encodeURIComponent(details)}&location=${encodeURIComponent(churchLocation)}&sf=true&output=xml`;
