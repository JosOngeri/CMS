...
run(login_cmd, "Test login through Caddy")

# Also check if user exists in DB
run("PGPASSWORD='Jos@123' psql -h localhost -U postgres -d cms_db -c \"SELECT id, username, email, is_active FROM users WHERE username ILIKE 'Admin'\"", "Check Admin user in database")
